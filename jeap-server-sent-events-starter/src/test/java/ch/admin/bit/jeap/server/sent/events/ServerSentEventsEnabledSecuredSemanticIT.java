...
class ServerSentEventsEnabledSecuredSemanticIT extends KafkaIntegrationTestBase {
    ...
    private static final SemanticApplicationRole OTHER_READ_ROLE = SemanticApplicationRole.builder()
            .system("test-system")
            .resource("other")
            .operation("read")
            .build();
    ...
    void authorized() throws Exception {
        String token = createAuthTokenForUserRoles(SSE_READ_ROLE);
    ...
    @Test
    void nonAuthorizedOtherToken() throws Exception {
        String token = createAuthTokenForUserRoles(OTHER_READ_ROLE);
        SSEClientTestSupport.SSEClient testClient = SSEClientTestSupport.createTestClient(baseUrl, "/ui-api/sse/events", 1, token);
